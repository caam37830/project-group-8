...
# Also set an IMG_PREFIX global variable so that plots get saved to an appropriate
# directory
...
ms = np.array([50, 50, 100, 100]).repeat(4)
ns = np.array([50, 100, 50, 100]).repeat(4)
ks = np.tile(np.array([0.1, 0.2, 0.3, 0.4]), 4)
ps = np.tile(np.array([0.1, 0.3, 0.5, 0.7]), 4)

# The following affect ONLY the initial state
prop_alive = np.tile(np.array([0.3, 0.4, 0.5, 0.6]), 4)
prop_infect = np.tile(np.array([0.1, 0.2, 0.3, 0.4]), 4)
for m, n, k, p, pa, pi in zip(ms, ns, ks, ps, prop_alive, prop_infect):
    # Simulate 100 days and save results to a gif file
    results = simulate_and_animate(100, m, n, k, p, pa, pi, IMG_PREFIX)
    # Print filename and results to stdout, so we can look at them later if we want
    print(
        "m = {}, n = {}, k = {}, p = {}, pia = {}, pii = {}".format(m, n, k, p, pa, pi)
    )
    print("-" * 80)
    print(results)
